...
import { downloadReportJson, downloadReportPdf } from '@/lib/api/endpoints';
    ...
    <Card className="print-plain overflow-hidden">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-hairline/70 px-4 py-4 sm:px-6">
        ...
        <div className="no-print flex flex-wrap gap-2">
          <Button variant="solid" size="sm" onClick={() => void downloadReportPdf(result.id)}>
            <FileDown size={13} /> Download PDF
          </Button>
          <Button variant="outline" size="sm" onClick={() => void downloadReportJson(result.id)}>
...
function ScanHistory() {
  const { history, historyLoading, removeHistoryEntry, openScanById } = useApp();
  ...
  return (
    <Card className="no-print">
      <CardHeader
        title="Scan History"
        subtitle={`${history.length} scan${history.length === 1 ? '' : 's'} saved to your account.${historyLoading ? ' Refreshing…' : ''}`}
      />
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1.5">
                        <Button
                          ...
                          onClick={() => void openScanById(h.id)}
                        >
                          View
                        </Button>
                        <Button
                          ...
                          onClick={() => void downloadReportPdf(h.id)}
                        >
                          Report
                        </Button>
                        <Button
  ...
  return (
    <div className="space-y-6">
      <SectionHeading
        ...
        description="The full report for the current scan, plus every scan saved to your account."
        action={
