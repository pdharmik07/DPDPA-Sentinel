...
/**
 * The tool must always distinguish a statutory requirement from a
 * project-specific check or a recommended practice.
 */
const SOURCE_LABEL: Record<string, string> = {
  ACT: 'DPDP Act 2023 — statutory',
  RULES_2025: 'DPDP Rules 2025 — statutory',
  PROJECT_SPECIFIC: 'Project-specific check',
  BEST_PRACTICE: 'Recommended practice',
};
                      ...
                      <td colSpan={5} className="px-4 py-4">
                        <div className="grid gap-4 lg:grid-cols-2">
                          <div className="space-y-3">
                            ...
                            <div>
                              <p className="label mb-1.5">Legal basis</p>
                              <div className="flex flex-wrap items-center gap-1.5">
                                <Badge tone={SOURCE_TONE[f.requirement.sourceType ?? 'ACT']}>
                                  {SOURCE_LABEL[f.requirement.sourceType ?? 'ACT']}
                                </Badge>
                                <span className="font-mono text-[0.62rem] text-ink-dim">
                                  {f.requirement.section}
                                </span>
                              </div>
                              {f.requirement.effectiveFrom ? (
                                <p className="mt-1.5 text-[0.68rem] leading-relaxed text-ink-faint">
                                  The cited DPDP Rules 2025 provision comes into force on{' '}
                                  <span className="text-ink-dim">{f.requirement.effectiveFrom}</span>.
                                </p>
                              ) : (
                                <p className="mt-1.5 text-[0.68rem] leading-relaxed text-ink-faint">
                                  Commencement date not established by the supplied sources.
                                </p>
                              )}
                            </div>
      ...
      <div className="grid gap-5 p-4 sm:p-5 lg:grid-cols-[auto_minmax(0,1fr)]">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-hairline bg-black/25 px-3 py-2.5">
            <p className="label mb-1">Scoring model</p>
            ...
            <p className="mt-1 font-mono text-[0.64rem] text-ink-faint">
              Points earned <span className="text-neon">{t.earnedWeight.toFixed(1)}</span> of{' '}
              <span className="text-neon">{t.applicableWeight.toFixed(1)}</span> across applicable requirements
            </p>
            {/* Points come from the Rule Engine Design Document. A partial
                mandatory rule scores 1, NOT half of 3 — stating "half weight"
                here was left over from the retired browser engine. */}
            <p className="mt-1.5 text-[0.64rem] leading-relaxed text-ink-faint">
              Pass / partial / fail — mandatory 3 · 1 · 0, conditional 2 · 1 · 0, recommended 1 · 0.5 · 0.
              Category score = points earned ÷ points available for its applicable rules.{' '}
              {t.notApplicable} requirement{t.notApplicable === 1 ? '' : 's'} did not apply to this document and
              {t.notApplicable === 1 ? ' was' : ' were'} excluded from scoring entirely.
            </p>
          </div>
