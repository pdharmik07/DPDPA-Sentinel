      ...
      <div className="relative grid gap-8 p-6 sm:p-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] lg:p-10">
        ...
        <div className="relative mx-auto grid w-full max-w-xs place-items-center lg:max-w-none">
          ...
          <p className="mt-3 text-center font-mono text-[0.6rem] uppercase tracking-[0.2em] text-ink-faint">
            {REQUIREMENT_COUNT} clause categories armed
          </p>
        </div>
      </div>
    </Card>
...
export default function Dashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    // Average each category across all scans for the coverage charts.
    const categories: CategoryScore[] = CATEGORIES.map((c) => {
      const values = history
        .flatMap((h) => h.categories.filter((cat) => cat.id === c.id))
        .map((cat) => cat.score);
      const score = values.length ? Math.round(values.reduce((a, b) => a + b, 0) / values.length) : 0;
      return { id: c.id, label: c.label, score, earned: score, possible: 100, requirements: values.length };
  ...
  const hasDemo = history.some((h) => h.demo);
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-4">
        <SectionHeading
          ...
          description="Aggregated across every scan stored in this browser."
          action={
            hasDemo ? (
              <Badge tone="violet">Includes sample data</Badge>
            ) : null
          }
        />
        ...
        {history.length > 0 ? (
          ...
          <Card className="p-8 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm text-ink-faint">
              ...
              {settings.seedDemoData ? '' : ' Sample data is disabled in Settings.'}
            </p>
